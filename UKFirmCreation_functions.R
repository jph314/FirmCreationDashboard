...
showMap <- function(d1, d2) {
  ...
  TNUTS <- registerPC[which(between(registerPC$date, d1, d2)),] %>% group_by(NUTS2) %>% count()
  # Draw map.
  ...
  plot_ly(mapNUTS) %>% 
    add_sf(color = ~n^0.6,
           ...
           type = 'scatter') %>% 
    layout() %>%
    colorbar(title = paste0('New registrations between<br>', d1, ' and ', d2), len = 0.5, x = 0.7, y = 0.65, showticklabels = FALSE)
...
drawTreemap <- function(df, d1, d2) {
  plot_ly(alpha = 0.5, data = df, ids = ~ids, labels= ~labels,
...
showTable <- function (tabledata) {
  ...
  datatable(tabledata, container = container_dt, rownames = F, class = "",
...
sectorPlot <- function(d1, d2, section, country) {
  ...
  plot_ly(data = df) %>% add_trace(x = ~date, y = ~avg, color = ~Section.name, colors = 'viridis',
                                     type = 'scatter', mode = 'lines', visible = TRUE,
                                     line = list(width = 0.8)) %>%
...
groupRegion <- function(d1, d2, group, a) {
  ...
  eng2 <- df$n[which(df$NUTS1 %in% list("UKC", "UKD", "UKE", "UKF", "UKG", "UKH", "UKJ", "UKK"))] %>%
    ...
    rename(n = x) %>%
    mutate(avg = frollmean(n, n=7))
  plot_ly() %>% add_trace(x = eng1$date, y = eng1$avg, name = "England (inc. Lon)",
                          type = 'scatter', mode = 'lines', visible = TRUE,
                          line = list(width = 0.8, color = '#440154')) %>%
    add_trace(x = eng2$date, y = eng2$avg, name = "England (exc. Lon)",
              type = 'scatter', mode = 'lines', visible = TRUE,
              line = list(width = 0.8, color = '#414487')) %>%
    add_trace(data = df[which(df$NUTS1 == "UKI"),], x = ~date, y = ~avg, name = "London",
              type = 'scatter', mode = 'lines', visible = TRUE,
              line = list(width = 0.8, color = '#2a788e')) %>%
    add_trace(data = df[which(df$NUTS1 == "UKM"),], x = ~date, y = ~avg, name = "Scotland",
              type = 'scatter', mode = 'lines', visible = TRUE,
              line = list(width = 0.8, color = '#22a884')) %>%
    add_trace(data = df[which(df$NUTS1 == "UKL"),], x = ~date, y = ~avg, name = "Wales",
              type = 'scatter', mode = 'lines', visible = TRUE,
              line = list(width = 0.8, color = '#7ad151') )%>%
    add_trace(data = df[which(df$NUTS1 == "UKN"),], x = ~date, y = ~avg, name = "Northern Ireland",
              type = 'scatter', mode = 'lines', visible = TRUE,
              line = list(width = 0.8, color = '#fde725')) %>% 
    add_trace(x = eng1$date, y = eng1$n, name = "England (inc. Lon)",
                                                      type = 'scatter', mode = 'lines', visible = FALSE,
                                                      line = list(width = 0.8, color = '#440154')) %>%
    add_trace(x = eng2$date, y = eng2$n, name = "England (exc. Lon)",
              type = 'scatter', mode = 'lines', visible = FALSE,
              line = list(width = 0.8, color = '#414487')) %>%
    add_trace(data = df[which(df$NUTS1 == "UKI"),], x = ~date, y = ~n, name = "London",
              type = 'scatter', mode = 'lines', visible = FALSE,
              line = list(width = 0.8, color = '#2a788e')) %>%
    add_trace(data = df[which(df$NUTS1 == "UKM"),], x = ~date, y = ~n, name = "Scotland",
              type = 'scatter', mode = 'lines', visible = FALSE,
              line = list(width = 0.8, color = '#22a884')) %>%
    add_trace(data = df[which(df$NUTS1 == "UKL"),], x = ~date, y = ~n, name = "Wales",
              type = 'scatter', mode = 'lines', visible = FALSE,
              line = list(width = 0.8, color = '#7ad151') )%>%
    add_trace(data = df[which(df$NUTS1 == "UKN"),], x = ~date, y = ~n, name = "Northern Ireland",
              type = 'scatter', mode = 'lines', visible = FALSE,
              line = list(width = 0.8, color = '#fde725')) %>%
    layout(title = title,
           yaxis = list(title = "Number of registrations"),
