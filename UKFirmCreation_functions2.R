...
# Value boxes
vboxes <- function(vb, d1, d2, country, Tcountry, pickData) {
  if (vb == 1) {
...
# UK NUTS2 map ----
showMap <- function(d1, d2, pickData) {
  # Load shapefile.
  ...
  TNUTS <- registerPC[which(between(registerPC$date, d1, d2) & registerPC$archive == pickData), ] %>%
    group_by(NUTS2) %>%
    count()
  # Draw map.
...
NUTSdata <- function(d1, d2, pickData) {
  # Aggregate registrations by NUTS2 in selected date range from full dataset.
  TNUTS <- registerPC[which(between(registerPC$date, d1, d2) & registerPC$archive == pickData), ] %>%
    group_by(NUTS218NM) %>%
    count()
