...
server <- function(input, output, session) {
  ...
  Tcountry <- reactive({
    countrySelect(input$pickCountry, register(), input$dateAgg[1], input$dateAgg[2])
  })
  ...
  #   vboxes(2, as.Date("2020-12-08"), input$dateAgg[2], Tcountry())
  # )
  # output$vboxLD <- renderValueBox(
  #   vboxes(3, as.Date("2021-01-05"), as.Date("2021-04-11"), Tcountry())
  # )
  # output$vboxOpen <- renderValueBox(
  #   vboxes(4, as.Date("2021-04-12"), input$dateAgg[2], Tcountry())
  # ) # VBoxes
  ...
  output$UKmap <- renderPlotly({
    showMap(register(), input$dateAgg[1], input$dateAgg[2])
  }) # UK NUTS2 map
  ...
  Tdivision <- reactive(
    ...
    Tcountry()$n %>%
      aggregate(by=list(SectionAbb = Tcountry()$SectionAbb, Division.name = Tcountry()$Division.name),sum) %>%
      rename(n=x)
  )
  ### Sector tree map
  sunburstDF <- reactive({
  ...
  ### Regional plots ----
  Tlad <- reactive({
    ladSelect(Tcountry(), input$ladPicker)})
  # Selected local authority districts in a table.
  output$districts <- renderDT({
    ladTable(distinct(Tlad()[-c(1,5,6)], District, .keep_all = TRUE))
  }) # Districts table
